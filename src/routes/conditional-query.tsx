			...
			<div className="island-shell rounded-xl p-4">
				...
				<ul className="space-y-3">
					{result.data.map((user) => (
						<li key={user.id} className="rounded-lg border border-(--line) bg-(--surface) p-3">
...
const result = useQuery({
  ...
  enabled: !!debouncedSearch,  // Only fetch when debounced value exists
				...
				<div className="mb-6">
					<label className="mb-2 block text-sm font-medium text-(--sea-ink)">Search Users</label>
					<input
						type="text"
						value={search}
					...
					<p className="mt-2 text-xs text-(--sea-ink-soft)">
						Query is {debouncedSearch ? "enabled" : "disabled"} (enabled: {String(queryEnabled)})
					</p>
				</div>
